...
export function middleware(request: NextRequest) {
  // 0. Redirect non-www to www (canonical domain enforcement)
  const host = request.headers.get('host');
  if (host === 'tamizhtech.in') {
    return NextResponse.redirect(
      new URL(request.nextUrl.pathname + request.nextUrl.search, 'https://www.tamizhtech.in'),
      301
    );
  }
